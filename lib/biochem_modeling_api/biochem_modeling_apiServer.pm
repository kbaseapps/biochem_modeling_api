package biochem_modeling_api::biochem_modeling_apiServer;
...
our %method_authentication = (
        'search_reaction' => 'required',
        'search_compound' => 'required',
);
...
sub call_method {
    ...
    my $ctx = biochem_modeling_api::biochem_modeling_apiServerContext->new($self->{loggers}->{userlog},
...
{
    # Service biochem_modeling_api requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for biochem_modeling_api but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = biochem_modeling_api::biochem_modeling_apiServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service biochem_modeling_api.");
    }
...
package biochem_modeling_api::biochem_modeling_apiServerContext;

use strict;

=head1 NAME

biochem_modeling_api::biochem_modeling_apiServerContext
...
package biochem_modeling_api::biochem_modeling_apiServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = biochem_modeling_api::biochem_modeling_apiServer->new(
